<?php $dsn = 'データベース名';
      ...
      //データベース接続
      $sql="CREATE TABLE IF NOT EXISTS mission_5"//ここからテーブルの生成
      ."("
...
<?php $h_num="";
      ...
      $nvar=0;
      $row='';
      if(!empty($_POST["enum"])){//ここから編集識別機能
          //enumが空じゃない時に以下を実行
          $id=$_POST["enum"];
          $id=(int)$id;
          $pass=$_POST["epass"];
...
<html lang="ja">
    ...
    <body>
        <form method="post" action="">
          ...
          【削除欄】<br>
          <input type="number" name="dnum" placeholder="削除したい番号を入力"><br>
          <input type="text" name="dpass" placeholder="設定したパスワードを入力"><br>
          <input type="submit" name="delete" value="削除実行"><br><br>
          【編集欄】<br>
          <input type="number" name="enum" placeholder="編集したい番号を入力"><br>
          <input type="text" name="epass" placeholder="設定したパスワードを入力"><br>
          <input type="submit" name="edit" value="編集呼び出し"><br><br>
         </form>
         【投稿内容】
          <hr>
    </body>
...
<?php //ここから編集識別機能のブラウザ表示
      if($var==1){//☆投稿番号＆パスワードが正しいとき
          echo "【投稿欄に編集内容を記入してください】<br>パスワードを再設定してください。<br><br>";
      }elseif($nvar==1){//投稿番号が存在しない時の表示
          echo "【※エラー：投稿番号が存在しません】<br><br>";
      }elseif($nvar==2){
          echo "【※エラー：編集欄に投稿番号が入力されていません】<br><br>";
      }elseif($var==2){//☆投稿番号〇だが、パスワードが違う時
          echo "【※エラー：パスワードが正しくありません】<br><br>";
      }elseif($var==3){//☆投稿番号〇だが、パスワードが入力されていない時
          echo "【※エラー：パスワードを入力してください】<br><br>";
      }//ここまで編集識別機能のブラウザ表示
      
      if (!empty($_POST["name"])) {//ここから編集実行機能＆通常投稿機能
           if(!empty($_POST["comment"])){
             if(!empty($_POST["pass"])){
               if(!empty($_POST["h_num"])){//ここから編集実行機能
                  $id=$_POST["h_num"];
                  $name=$_POST["name"];
                  ...
                  $stmt->execute();
                  //sql（編集）を実行
                  echo "【編集が完了しました】<br><br>";//ここまで編集実行機能
         ...
         //投稿欄の名前が空で、削除欄と編集欄に何も書かれていない時…
         echo "【※エラー：名前を入力してください】<br><br>";
     }//ここまで通常投稿
           
                   
      if(!empty($_POST["dnum"])){//ここから削除機能
          if(!empty($_POST["dpass"])){
                ...
                foreach($result as $row){
                      if($row["pass"]==$pass){
                          ...
                          $sql= 'delete from mission_5 where id=:id AND pass=:pass';
                          $stmt=$pdo->prepare($sql);
                          $stmt->bindParam(':id',$id, PDO::PARAM_INT);
                          $stmt->bindParam(':pass',$pass, PDO::PARAM_STR);
                          $stmt->execute();
      ...
      $sql='SELECT * FROM mission_5';//結果をブラウザに表示する↓
      $stmt=$pdo->query($sql);
      $result=$stmt->fetchAll();
      foreach($result as $row){
              echo $row['id'].' ';
